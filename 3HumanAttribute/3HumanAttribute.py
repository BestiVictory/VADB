...
from dataloaders.dataloaders_human import DATALOADER_DICT
...
def get_args(description='CLIP4Clip for Visual Output Extraction'):
    ...
    parser.add_argument('--val_csv', type=str, default='3human_attribute_csv/val.csv', help='')
...
class IntegratedAestheticPredictor(nn.Module):
    def __init__(self, input_dim, hidden_dims=[512, 256], num_branches=4):
...
def set_seed_logger(args):     #set_seed_logger函数用于接收一个参数args
    ...
    args.output_dir = "3train_human_attribute/output_dir_0508-model0506"
...
def predict_video_scores(model, aesthetic_model_path, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Predicting"):
            ...
            scores = torch.stack(scores, dim=1).float()  # [batch_size, 5]
            ...
            # 使用集成模型预测所有分支的分数
            predicted_scores = integrated_model(visual_output)  # [batch_size, 5]
    ...
    # 合并结果
    predicted_scores = np.concatenate(all_predicted_scores, axis=0)  # [num_samples, 5]
    true_scores = np.concatenate(all_true_scores, axis=0)  # [num_samples, 5]
    
    return all_video_ids, predicted_scores, true_scores

def save_predictions_to_csv(video_ids, predicted_scores, true_scores=None, output_file="predictions.csv"):
    attribute_names = [ '表情', '动作', '服装', '化妆']
...
def evaluate_predictions(true_scores, predicted_scores):
    ...
    for branch in range(4):  # 修改为4，因为你只有4个属性
        true = true_scores[:, branch]
        pred = predicted_scores[:, branch]
        ...
        srocc, _ = spearmanr(true, pred)
        plcc, _ = pearsonr(true, pred)
        krcc, _ = kendalltau(true, pred)  # 新增KRCC计算
...
def main():
    ...
    # 模型路径
    aesthetic_model_path = "3train_human_attribute/output_dir_0508-model0506/IntegratedAestheticPredictor_epoch5_avg_test_loss_0.6309_best.pth"
    # 确保输出目录存在
    result_dir = "3train_human_attribute/val_result"
    ...
    # 保存预测结果
    output_csv = os.path.join(result_dir, "predict_human_test.csv")
